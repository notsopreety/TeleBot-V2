...
module.exports = {
    ...
    onStart: async function({ msg, bot, args }) {
        try {
            if (args.length === 0) {
                return bot.sendMessage(msg.chat.id, 'No URL provided. Usage: /pindl <URL>');
            }
            ...
            if (data && data.medias && data.medias.length > 0) {
                ...
                if (videoMedia) {
                    ...
                    await bot.sendVideo(msg.chat.id, videoUrl, { caption: `<b>Title:</b> ${message}`, parseMode: 'HTML' });
                } else if (imageMedia) {
                    const imageUrl = imageMedia.url;
                    await bot.sendPhoto(msg.chat.id, imageUrl, { caption: `<b>Title:</b> ${message}`, parseMode: 'HTML' });
                }
            } else {
                await bot.deleteMessage(downloadMsg.chat.id, downloadMsg.message_id);
                bot.sendMessage(msg.chat.id, 'No media found! Something went wrong.');
            }
        } catch (error) {
            console.error('Error fetching or sending media:', error);
            bot.sendMessage(msg.chat.id, 'Error fetching or sending media.');
        }
    }
};
