...
const openWeatherAPI = async function (q, type) {
    try {
        ...
        switch (type) {
            case "geo":
                ...
                info = await axios.get(
                    `https://api.openweathermap.org/data/2.5/weather?lat=${geo[0]}&lon=${geo[1]}&${encodeQueryParams({ appid: openWeather, units: "metric", lang: "id" })}`
                );
                break;
            case "city":
                info = await axios.get(
                    `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(q)}&${encodeQueryParams({ appid: openWeather, units: "metric", lang: "id" })}`
                );
                break;
        }
        if (info.status === 200) {
        ...
        console.error("Error in openWeatherAPI:", e);
        return { status: 500, msg: "Error fetching weather information" };
    }
};
